  ...
  if (authLoading || (loading && bookings.length === 0)) {
    return (
      <div className="min-h-screen pt-32 flex items-center justify-center">
        <LoadingSpinner size={48} color="#e11d48" />
      </div>
    );
  }
    ...
    <div className="min-h-screen pt-32 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-900 text-[10px] font-black text-white mb-6 rounded-full border border-slate-800 uppercase tracking-widest">
              <ShieldCheck size={12} fill="white" /> Command Center
            </div>
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-slate-900 leading-[0.8] mb-4">
              <DecryptedText text="Admin Control." speed={80} maxIterations={20} sequential />
            </h1>
            <p className="text-xl text-slate-500 font-medium max-w-xl">
              Manage Nakuru's elite performance network. Oversee bookings, handle inquiries, and broadcast system updates.
            </p>
          </div>

          {/* Quick Stats */}
          <div className="flex gap-4">
            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm min-w-[160px]">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Pending Bookings</p>
              <h4 className="text-3xl font-black text-rose-600">{bookings.filter(b => b.status === 'pending').length}</h4>
            </div>
            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm min-w-[160px]">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">New Inquiries</p>
              <h4 className="text-3xl font-black text-blue-600">{inquiries.filter(i => i.status === 'pending').length}</h4>
            </div>
          </div>
        </div>
              ...
              {activeTab === 'bookings' && (
                ...
                >
                  ...
                  {bookings.length > 0 ? bookings.map(booking => (
                    <div key={booking.id} className="p-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-rose-100 transition-colors">
                      <div className="space-y-1">
                        ...
                        <p className="text-slate-500 text-sm font-medium flex items-center gap-2">
                           <Activity size={14} className="text-rose-500" /> {booking.plan} • {booking.location}
                        </p>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        {inquiry.status !== 'responded' && (
                          <button 
                            ...
                            className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-600 transition-all flex items-center gap-2"
                          >
                    ...
                    <form onSubmit={handleCreatePromo} className="space-y-8">
                      <div className="grid md:grid-cols-2 gap-8">
                        <div className="space-y-3">
                          ...
                          <input 
                            ...
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 focus:outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all font-bold text-slate-800"
                          />
                        </div>
                        <div className="space-y-3">
                          ...
                          <select 
                            ...
                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 focus:outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all font-bold text-slate-800 appearance-none"
                          >
                      ...
                      <div className="space-y-3">
                        ...
                        <textarea 
                          ...
                          className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 focus:outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 transition-all font-bold text-slate-800 resize-none"
                        ></textarea>
                      </div>

                      <button 
                        disabled={isSubmittingPromo}
                        className="w-full bg-rose-600 text-white font-black py-6 rounded-[2rem] shadow-xl shadow-rose-600/20 hover:bg-rose-700 transition-all active:scale-[0.98] flex items-center justify-center gap-3 text-lg uppercase tracking-widest disabled:opacity-50"
                      >
                        {isSubmittingPromo ? (
                          <>
                            <LoadingSpinner size={20} color="white" /> Dispatching...
                          </>
                        ) : (
                          <>
                            <Zap size={20} fill="white" /> Dispatch Promotion
                          </>
                        )}
                      </button>
                    </form>
